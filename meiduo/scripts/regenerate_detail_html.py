...
# 使用当前项目指定或终端进入  , 可以是相对路径也可以是绝对路径，
# 相对路径在以此文件作为脚本执行时， 必须进入这个项目的拟环境，否则找不到这个解释器，
# 另外必须注意python后的回车符号是\r（Ubuntu） 还是\r\n（Win）才正确

#  将外层meiduo加入到导包路径（因为已经不是manage.py启动， meiduo 不再是导包路径， scripts才是， 故要新加进来）
sys.path.insert(0, '../')
...
# 以下的文件的导包路径是依靠meiduo文件夹的， 和django项目的，必须放在新增导包路径和启动django之后， 否则报错
from django.template import loader
from django.conf import settings

from goods import models
from contents.utils import get_goods_channel, get_breadcrumb
